...
[Answer]: X — Deepgram を Lex v2 のカスタム音声モデル（ASR）として統合。TTS は Amazon Polly を使用。NLU は Lex v2 が担当。
...
[Answer]: X — DynamoDB 保持は 3 ヶ月。コンタクト終了時に CRM へ API 連携で自動書き込みし、CRM 側で 10 年保管（CRM 連携はスコープ外）。
...
[Answer]: C — AWS Amplify + React。ランニングコストを最小化する設計とする。
...
[Answer]: B — 新規インスタンスを CDK で完全新規プロビジョニング。
...
[Answer]: C — 大規模（ページ 500 以上・FAQ 500 件以上）
