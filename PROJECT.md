...
電話・チャットの問い合わせを、公開ウェブ情報（HTML ページ由来の ~13,500 チャンク。
PDF/画像の非HTMLノイズと http/https 重複は除外）に基づき Claude が回答し、答えられない
場合は人間オペレーターへエスカレーションします。
...
- **コーパス浄化** — クローラーの非HTMLスキップ導入前に埋め込まれた PDF/画像チャンクが
  残留し、コーパスの **84%（13万→そのうち約11.5万）**を占めて本命の HTML を希釈していた
  （FX アプリ手動 PDF 1本で約5,900チャンク）。検索診断で特定し一括パージ。さらにクローラーが
  http/https 両方を辿り**同一ページを二重登録**（コーパスの約半分）していた問題を URL の https
  正規化＋既存重複削除で解消し、旧 Shift_JIS ページの文字化けも bytes→BeautifulSoup で修正。
  結果 **HTML 由来の ~13,500 チャンク**（ノイズ・重複・文字化け排除）に。参照元は生 URL でなく
  **ページタイトル**で表示。
